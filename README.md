...
このプロジェクトは、研究室のインターンシップ課題として開発されました。

CSV形式のEMG（筋電図）データを読み込み、様々な信号処理フィルタをインタラクティブに適用しながら、時間領域と周波数領域でリアルタイムに結果を可視化することを目的としています。

モジュール単位で実際にプログラムを組みながら信号処理アプリケーションを作成しましょう！
